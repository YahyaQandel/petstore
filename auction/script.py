...
def sort_2(bidders_sorted_desc):
    i = 0
    bidders_sorted_list = []
    for bidder in bidders_sorted_desc:
        list_item = []
...
def get_what_bidders_should_pay(file):
    ...
    if len(bidders) == 0:
        return "No Winners"
    bidders_sorted_desc = create_bidders_list(bidders)
    bidders_sorted_desc.sort()
